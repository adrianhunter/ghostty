...
pub fn build(b: *std.Build) !void {
    ...
    //     &deps,
    // );
    // _ = libghostty_shared; // autofix
    // const libghostty_static = try buildpkg.GhosttyLib.initStatic(
    //     b,
    //     &deps,
    // );
    // _ = libghostty_static; // autofix
    const exeWasm = try buildpkg.GhosttyWasm.init(b, &config, &deps);
    ...
    if (config.target.result.cpu.arch.isWasm()) {
        try emsdk.configure(.{
            ...
            .extra_args = &.{
                ...
                // "-sEXPORTED_FUNCTIONS=_valueDeinit",
            },
            .builder = b,
            .deps = &.{
                .{
